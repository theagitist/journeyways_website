...
- **`brand-spec.md` is publicly served** at `https://www.journeyways.ca/brand-spec.md` as `text/plain; charset=utf-8` with `Cache-Control: public, max-age=86400`. Requires a per-site nginx exception: an exact-match `location = /brand-spec.md` block placed **before** `include snippets/security-hardening.conf;` in the vhost, because the security snippet 404s every `.md` file by default as a defense-in-depth measure. The vhost block uses `default_type "text/plain; charset=utf-8"` plus `try_files /brand-spec.md =404`. Footer link on every page (small, gray-600, next to the version tag) makes it discoverable. Treat it as the canonical brand reference for the site, print derivatives, and the videogame UI; downstream projects (e.g. `play.journeyways.ca`) should reference this URL rather than copying the file. The full doc covers the visual identity (logo + usage rules, palette, typography, components, voice + examples, tokens), editorial chapter spine pattern, the presentation slide system, per-page hero variations, and §9 Companion materials (print + videogame).
