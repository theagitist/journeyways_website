...
- **`presentation.html` is the conference deck** (renamed from `pitch.html` in May 2026; "pitch" was deemed too marketing-y). `noindex, nofollow` (linkable from the site, not indexed by search engines). Linked from the About hero bar between References and UBC GRSJ; included in the sitemap with priority 0.5. Currently 15 slides, refactored in May 2026 to mirror the narrative arc of the JOURNEYWAYS conference video (cover, problem-as-contrast, proposition, the three-component instrument with one slide per component plus an "inside a card" slide, nine principles, physical/digital peers, ethics, four-phase research arc, what-has-surfaced + vignettes combined, framework + governance, combined closing-and-thanks). Reference materials live in `~/FileShare/jw/pitch/` (mp4 video, pptx slides, m4a discussion, infographic png) — gitignored. Self-contained: inline `<style>` defines the slide system, inline `<script>` handles keyboard navigation (arrows, space, Home/End, F fullscreen, S speaker notes, P print). Speaker notes hide by default; toggle with `S` or open with `?notes`. Print stylesheet (`@media print` with `@page { size: 16in 9in }`) lays each slide out as a 16:9 page for "Print to PDF". The deck reuses the editorial pattern (Italianno chapter titles in `text-yellow-400`, watercolor swatches from `img/design/bg-*.webp`, `border-t border-gray-700/40` hairlines) but does NOT include the standard nav; only a top-left "← JOURNEYWAYS" link back to home, plus a slide counter top-right. The bottom keyboard-hint legend auto-fades after 3 seconds of inactivity and reappears on mouse/keyboard activity. The `.timeline.four` CSS variant supports the four-phase methodology grid on slide 12. Slide 7 uses `.card-front-row.four` to fit four card examples with their hand-authored prompt text. Card-pile labels on slide 6 use the `.card-COLOR` classes from `css/styles.css`, with overrides in the inline `<style>` to beat the white-default specificity of `.card-back .label`.
