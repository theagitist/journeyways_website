...
├── pitch.html              Standalone conference deck (HIDDEN: noindex/nofollow, off the nav, off the sitemap, no inbound links). 19 slides; self-contained inline CSS+JS, no external slide framework. Reachable only by direct URL while we iterate on content
...
├── sitemap.xml             7 entries with <lastmod>; contact.html and pitch.html intentionally absent
