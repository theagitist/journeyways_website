...
- **Site-operational TODOs** live in the vault at `~/apps/obsidian/Academia/Journeyways/ToDo/www.md`. Single source of truth for marketing-site work-to-do; check there first when looking for open items. The sibling `ToDo/play.md` plays the same role for `play.journeyways.ca`. Project-level commitments (boardgame v2 content, BREB milestones, videogame feature work) stay in their ROADMAP files under `Boardgame Specific/Documentation/` and `Videogame Specific/Documentation/`; the ToDo notes are for operator and infrastructure work.
- **Governance docs are not in this repo.** Brand spec, boardgame compliance / privacy / data-governance / production / playtesting / roadmap all live in the Academia vault. The marketing site temporarily has no governance docs publicly accessible (since 2026-05-12); restoration via an HTML rendering pipeline is the top item in `ToDo/www.md`.
