...
- **Brand spec** (no longer in this repo as of 2026-05-12). Canonical copy moved to the Academia Obsidian vault at `Journeyways/Foundations/brand-spec.md`. Covers logo + usage rules, palette, typography, components, voice + examples, tokens, the editorial chapter spine pattern, the presentation slide system, hero variations per page, and §9 Companion materials covering print and the videogame. Not currently public; will return to the live site in HTML form when the vault-to-HTML rendering pipeline ships (queued in the vault's marketing roadmap).
...
- **Presenter window.** Press `W` from the main deck to pop open `?presenter=1` in a separate browser window. The presenter window shows the current slide's chapter title and h2, the full speaker notes, the next slide's chapter and title, a slide counter, and a session timer (`R` resets it; `Esc` closes the window). Both windows stay synced via `BroadcastChannel('journeyways-deck')` — arrow keys and digit jumps in either window advance both. Falls through gracefully if `BroadcastChannel` isn't available (presenter still works alone).
- **Slide overview.** Press `O` to open a full-screen grid of all 15 slide cards showing chapter title and h2. Click a card or press `Esc` / `O` to close. The overview is built at init by reading each slide's elements, so it stays in sync with edits.
- **Number-key navigation.** Type a slide number to jump there. Multi-digit input is buffered for ~700ms so "1" then "0" reaches slide 10. The buffer commits early when no longer-number could still fit (`buffer * 10 > total`). Out-of-range numbers clamp to the first or last slide.
- **Bottom hint legend reveals on cursor proximity only.** Hidden by default, fades in only when the cursor enters the bottom 64px of the viewport. No reveal on keypress or general mouse movement, which was distracting.
- **Speaker notes are properly readable.** The notes h4 switched from Italianno script (which fell apart at ~1.4rem) to a yellow uppercase Inter eyebrow line. Body text bumped to 1rem (inline) and clamp(1.05, 1.35vw, 1.25rem) (presenter window) at line-height 1.6. Every slide's notes expanded from terse cues into multi-bullet presenter blocks with concrete language to use, transitions, and specific examples.
- **Slide content tightened, more visual.** Slide 4 columns gained tile, card, and Player-Booklet thumbnails (the booklet thumb crops the labelled stack from `protoconbc_table.webp` with `transform: scale(2.7)` plus a contrast / saturation filter). Slide 5 grew from three to four tiles in a single row. Slide 6 colour-codes the Red / Green / Blue / Purple / Black labels via the site-wide `.card-COLOR` classes from `css/styles.css` (with specificity overrides in the inline style to beat the white-default of `.card-back .label`). Slide 7 expanded to four card examples with their hand-authored prompt text from `Cards.docx`. Slide 13 vignettes get banner thumbnails of Mirror Lake and Star Bridge to anchor the abstract stories. Slide 9 / 11 lists pick up yellow circular bullets via a `::before` pseudo. Cited authors (Leavy, McDermott, Ritterfeld; Butler, Serano, Stryker) link to the right `references.html` chapter anchors.
- **Closing slide cleaned and bookended.** Removed "Questions welcome" and "Visibility, where there has been silence." Added a small QR code (`img/qr-journeyways.svg`, generated white-on-`#111827` via `qrencode --foreground=ffffff --background=111827` so it sits flush against the slide background). The closing-mark uses a faded `og-card.webp` echo of the cover artwork as a bookend. Link list (`journeyways.ca · References · Design · UBC GRSJ`) moved to the very bottom of the slide.
